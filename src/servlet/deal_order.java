	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(session.getAttribute("user_id")!=null) {
			...
			if ("头等舱".equals(grade)) {
				...
				conn.executeInsert("update flight set f_f_c_n = f_f_c_n - 1 where f_n = '" + f_i +"'");
			} else if ("商务舱".equals(grade)) {
				db_conn conn=new db_conn();
				conn.executeInsert("update flight set f_s_c_n = f_s_c_n - 1 where f_n = '" + f_i +"'");
			} else if ("经济舱".equals(grade)) {
				db_conn conn=new db_conn();
				conn.executeInsert("update flight set f_t_c_n = f_t_c_n - 1 where f_n = '" + f_i +"'");
			}
			/*
